...
import com.zxf.security.core.captcha.CaptchaProcessor;
...
public class SmsCaptchaFilter extends OncePerRequestFilter implements InitializingBean {
    ...
    public void afterPropertiesSet() throws ServletException {
        ...
        String[] configUrls = StringUtils.splitByWholeSeparatorPreserveAllTokens(securityProperties.getCaptcha().getSms().getUrl(), ",");
    ...
    private void validate(ServletWebRequest servletWebRequest) throws ServletRequestBindingException, CaptchaException {
        ...
        String name = CaptchaProcessor.SESSION_KEY_PREFIX + "SMS";

        /*生成的captcha*/
        Captcha captchaInSession = (Captcha) sessionStrategy.getAttribute(servletWebRequest, name);

        /*用户输入的captcha*/
        String captchaInRequest = ServletRequestUtils.getStringParameter(servletWebRequest.getRequest(), "smsCaptcha");
        ...
        if (captchaInSession.isExpried()) {
            sessionStrategy.removeAttribute(servletWebRequest, name);
